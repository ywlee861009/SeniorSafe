...
- ✅ google-services plugin 활성화 (`android/app/build.gradle.kts` 주석 해제)
- ✅ `google-services.json`(`android/.gitignore`)·서비스 계정 JSON(`*-firebase-adminsdk-*.json`)·Supabase CLI 캐시(`supabase/.temp/`) git 제외
- ✅ `android/app/google-services.json` 로컬 배치 완료 — 실제 파일은 gitignore 대상
...
- ✅ `ANBU_API_BASE_URL`은 저장소 기준 실 Supabase Functions URL로 설정됨. 로컬/개인 환경은 Gradle property로 override 가능
...
- ✅ 2026-06-03 `cd android && ./gradlew assembleDebug` 통과
