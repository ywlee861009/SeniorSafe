...
로그인을 제거해도 FCM은 계속 핵심이다. 보호자 기기가 페어링 후 낙상 알림을 받으려면 Firebase 설정, FCM 토큰 등록, Android 알림 권한 처리가 실제 기기에서 검증되어야 한다.
...
- Android 14 이상 Foreground Service 권한과 실행 동작을 검증한다.
- 로컬/에뮬레이터/실기기 실행 가이드를 작성한다.

## 완료 조건

- 코드 수정 없이 local/dev/prod API URL을 바꿀 수 있다.
- 보호자 기기가 페어링 후 실제 FCM 낙상 알림을 받는다.
